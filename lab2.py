...
with DAG(
    ...
    def fetch_data(symbols=['AAPL', 'TSLA']):
        try:
            ...
            # Log the environment variables to check for None values
            for key, value in dbt_env.items():
                if value is None:
                    logging.error(f"Environment variable {key} is None. Please set it in your Airflow connection.")
    ...
    def insert_data_into_snowflake(results):
        try:
            ...
            with conn.cursor() as cur:
                ...
                # Insert data into the temporary staging table
                insert_sql = """
                ...
                # Perform MERGE to upsert data into the target table
                merge_sql = """
